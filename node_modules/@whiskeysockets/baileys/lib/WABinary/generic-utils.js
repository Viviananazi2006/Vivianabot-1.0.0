...
//=====[ CORREÇÃO FEITA POR MATHEUS CRIADOR DA NAKATUNY BOT E DODOCO BOT=====\\

const extractGroupMetadata = (group) => {
  if (!group || !group.attrs || !group.attrs.subject) {
    console.log('Objeto group ou suas propriedades não estão definidos corretamente.');
    return null;
  }
...
const assertNodeErrorFree = (node) => {
  ...
  if (errNode) {
    ...
    if (errorCode === 429) {
      console.log('rate-overlimit corrigido por: Matheus | Criador da Nakatuny/dodoco');
